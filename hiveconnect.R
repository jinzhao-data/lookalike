...
chart <- filter(data1,t1.cd < 50 )%>% ggplot(aes(x = t1.cd,y = g,colour = as.factor(t1.dt))) + geom_line()
p <- ggplotly(chart)
p
ggedit(chart)
library(RJDBC)
...
conn<-dbConnect(drv, sprintf('jdbc:hive2://192.168.2.47/:8888/default'), 'jinzhao', 'mp!jinzhao')
